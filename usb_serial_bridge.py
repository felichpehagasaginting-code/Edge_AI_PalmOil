...
# Example log: [MAIN] === SCAN #42 COMPLETE | Grade=1 (Matang) | Confidence=94% ===
DEBUG_LOG_PATTERN = re.compile(
    r"SCAN\s+#(\d+)\s+COMPLETE\s*\|\s*Grade=(\d+)\s*\((.*?)\)\s*\|\s*Confidence=(\d+)%"
)

def parse_args():
    parser = argparse.ArgumentParser(description="MAX78000FTHR to MQTT Serial Bridge")
    parser.add_argument(
        "-p", "--port",
        required=True,
    ...
    parser.add_argument(
        ...
        default=0,
        metavar="SECONDS",
        help="Auto-trigger scan every N seconds (0 = disabled, keyboard only)"
    )
...
def main():
    ...
    if args.auto_scan > 0:
        ...
        print(f"Keyboard:    press 's' + Enter to trigger scan remotely")
    print("-" * 60)
    ...
    try:
        ...
        mqtt_client.publish(MQTT_TOPIC_STATUS, json.dumps(status_payload), qos=0, retain=True)
    ...
    try:
        while True:
            if not ser.is_open:
                print("[SERIAL] Port closed unexpectedly. Retrying connection...")
                time.sleep(2)
                try:
                    ser.open()
                    continue
            ...
            try:
                ...
                if args.mode == "debug":
                    ...
                    if match:
                        ...
                        print(f"\n>>> [PARSED SCAN #{scan_count}] Grade={grade} ({grade_name}) | Confidence={confidence}%")
                
                # ── Mode B: Parsing raw JSON from UART0 (via FTDI bridge) ──────────
                elif args.mode == "raw":
                    # Looks like: {"g":1,"c":94}
                    if line.startswith("{") and line.endswith("}"):
                        try:
                            ...
                            if "g" in data and "c" in data:
                                ...
                                print(f"\n>>> [PARSED RAW] Grade={grade} | Confidence={confidence}%")
                        except ValueError:
                            pass
                ...
                if grade is not None and confidence is not None:
                    ...
                    print(f"[MQTT PUBLISH] Sent to {MQTT_TOPIC_RESULT}: {payload}\n")
